...
import { subscribeAuth, loginWithGoogle, logout } from '@/lib/auth'
...
export default function AdminPage() {
  ...
  useEffect(() => {
    return subscribeAuth(async (u) => {
      setUser(u)
      if (u) {
        const qs = await getQuizzesByHost(u.uid)
        setQuizzes(qs)
      }
      setLoading(false)
    })
  ...
  async function handleLaunch(quizId: string) {
    if (!user) return
    setLaunching(quizId)
    const roomId = await createRoom(quizId, user.uid)
    ...
    return (
      <main className="min-h-screen flex flex-col items-center justify-center gap-6 px-4">
        <div className="text-center">
          ...
          <p className="text-gray-400">Iniciá sesión para crear y lanzar quizzes</p>
        </div>
        <button
          onClick={loginWithGoogle}
          className="bg-white text-gray-900 font-bold px-8 py-4 rounded-xl hover:bg-gray-100 transition-colors flex items-center gap-3 text-lg"
        >
